...
2. Verify the remote branch contains the work to hand off:
   ```bash
   git status --short
   git rev-parse --abbrev-ref --symbolic-full-name '@{upstream}'
   git rev-list --count '@{upstream}..HEAD'
   ```
   Fargate clones from GitHub, so uncommitted or unpushed work will be absent. Ask before committing or pushing on the user's behalf.
...
4. Poll `aws ssm get-command-invocation` until the status is `Success` or `Failed`. The setup can take up to 15 minutes, so do not treat a short client-side polling timeout as task failure.
...
6. Report back to the user:
   ...
   - The task short ID.
   - Telegram controls: `/inspect <short-id>`, `/transcript <short-id>`, `/pi_prompt <short-id> <message>`, `/abort <short-id>`, and `/kill <short-id>`.
   - The S3 artifact prefix: `s3://coding-agent-session-logs-526094459325-af-south-1/sessions/<short-id>/`.
   - `manifest.json` is uploaded last and is the completion marker for transcripts, subagent artifacts, hashes, and token/cost usage. Artifacts expire after 30 days.

## Optional follow-ups

Use `hermes-fargate list`, `inspect`, `transcript`, `prompt`, `abort`, or `kill` through SSM. For example, list active sessions with:
```bash
